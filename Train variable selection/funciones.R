...
show_results2=function(folder){
  
  
  temp = list.files(folder)
  strategy = lapply(paste0(folder,'/',temp), read_csv)
  ...
  con2=strategy%>%
    mutate(expected=ifelse(prob<(0.50-esp),'act_2',ifelse(prob>(0.5+esp),'act_1','no des')))%>%
    count(expected,label)%>%
    group_by(expected)%>%
    mutate(p=n/sum(n))%>%
    ungroup()%>%
    mutate(label=fct_rev(label))%>%
    mutate(text=paste(round(p*100,3),n))%>%
    ggplot(aes(label,expected,fill=p,label=text))+
    geom_tile()+geom_text()+
    ggtitle('Tasa de acierto con indiferencia entre el% 40 y 60%')
  ...
  return(list(gtime=gtime,gbar=gbar,
              groc=NULL,
              ktable=ktable,
              con1=con1,con2=con2,datos_x=datos_x,
              data=retorno_portafolio))
